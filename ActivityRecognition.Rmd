...
The accuracies of each of these models are as follows:
...
The accuracies for some of theabove models seem high, and for that reason we'll use all of the above, to try to avoid overfitting.

The last step here is to fit a final model based on the `models` list. For that model we will use a random forest model model. In order to do that, we will need a new data frame of predictions.

```{r gbmModel, cache = TRUE, echo = TRUE}
library(dplyr, warn.conflicts = FALSE)
predictionDf <- lapply(models, function(m) predict(m, training) ) %>%
  as.data.frame %>%
  mutate(classe = training$classe)
modFinal <- train(classe ~ ., data = predictionDf, method = "rf", verbose = FALSE)
```
...
```{r perfAndError, cache = TRUE, echo = TRUE, message = FALSE, warning = FALSE}
library(dplyr, warn.conflicts = FALSE)
print(confusionMatrix(modFinal))
...
The in-sample error (1 - accuracy) here is 0.02%, but the out-of-sample error is much higher at 33%. 
...
### Discussion
There hasn't been a huge effort towards variable selection. It was generally assumed that the varibles chosen here yield acceptable models, an assumption that worked out well (with the possible exceptions of `gyros_belt` and `accel_belt`). The out-of-sample error rate is quite high, and more care could be taken with variable selection.
